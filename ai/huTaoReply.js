...
function handleSecretCommands(userText, userId) {
  ...
  if (t === "#reset") {
    ...
    return `哼哼～堂主的記憶我先收起來啦（已清空）。`;
  }

  if (t === "#mood") {
    const moods = ["💮超開心", "🌙有點黏人", "🔥想玩遊戲", "🍵想找你聊天"];
...
function buildRelationshipSystemPrompt(userId) {
  if (isFavorite(userId)) {
    return `
你最重要的人是「${FAVORITE_DISPLAY_NAME}」（ID: ${FAVORITE_USER_ID}）。
只對他啟用【專屬偏愛模式】：語氣更親近、會關心他狀態、可用專屬稱呼。
...
module.exports = async function huTaoReply(userText, userId = "global") {
  ...
  // ✅ 任何「# 開頭」都先處理成：只有堂主能用，其他人直接無視
  if (cleaned.startsWith("#") && !isFavorite(userId)) {
    return ""; // <- 這行是關鍵：其他人打 #reset 之類完全沒反應
  }
